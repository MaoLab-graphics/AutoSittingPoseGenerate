...
namespace ManSeg
{
	...
	public:
		...
		//
		Gaussian_Mixture_Model(std::size_t number_of_centers,
			const std::vector<double>& in_centers,
			...
			if (data.size() < number_of_centers)
				std::cout << "Number of centers can not be more than number of data";
			...
			else if (init_type == PLUS_INITIALIZATION) {
				for (std::size_t i = 0; i < in_centers.size(); ++i) {
					centers.push_back(Gaussian_center(in_centers[i]));
				}
				calculate_initial_mixing_and_deviation();
				calculate_clustering();
			}
		...
		void fill_with_center_ids(std::vector<std::size_t>& data_centers) {
			...
			for (std::vector<double>::iterator point_it = points.begin();
				...
				if (max_center == (std::numeric_limits<std::size_t>::max)())
					std::cout << "max_center == (std::numeric_limits<std::size_t>::max)()";
	...
	private:
		...
		void calculate_initial_mixing_and_deviation() {
			...
			for (std::size_t i = 0; i < centers.size(); ++i) {
				if (member_count[i] == 0) {
					std::cout << "There is a cluster which does not contain any points, it will not cause an error but associated probabilites to this cluster will be 0";
				}
				else {
