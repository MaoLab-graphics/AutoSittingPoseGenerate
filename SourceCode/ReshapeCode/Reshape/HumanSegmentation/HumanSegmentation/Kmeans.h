...
#define DEFAULT_MAXIMUM_ITERATION 10u
#define DEFAULT_NUMBER_OF_RUN 15u
#define DEFAULT_SEED 1340818006
	...
	public:
		...
		*/
		...
		void forgy_initialization(std::size_t number_of_centers,
			...
			for (std::size_t i = 0; i < number_of_centers; ++i) {
				// activate one more element in each iteration for as selectable
				std::size_t random_range = points.size() - number_of_centers + i;
				std::size_t random_index = std::rand() % (random_range + 1); // [0, random_range];
		...
		// To future reference, I also left prev implementation which is a variant of Fisher¨CYates shuffle, however to keep `points` intact I use another vector to
		...
		*/
		...
		void plus_plus_initialization(std::size_t number_of_centers,
			...
			std::size_t initial_index = std::rand() % (points.size() + 1); // [0, points size)
			...
			for (std::size_t i = 1; i < number_of_centers; ++i) {
				...
				if (total_probability == 0.0)
					std::cout << "Squared distances of all elements to their closest centers are 0.0, there is no way to select a new center.\n";

				double random_ds = (double)(rand() / (double)RAND_MAX / total_probability); // [0.0, total_probability)
	...
	public:
		...
		*/
		K_means_clustering(std::size_t number_of_centers,
			...
			std::size_t number_of_run = DEFAULT_NUMBER_OF_RUN,
			std::size_t maximum_iteration = DEFAULT_MAXIMUM_ITERATION)
			:
			points(data.begin(), data.end()),
			maximum_iteration(maximum_iteration),
			init_type(init_type) {
			if (TIME_SEED) std::srand((unsigned int)time(0));
			else std::srand(DEFAULT_SEED);
			
			if (data.size() < number_of_centers)
				std::cout << "Number of centers can not be more than number of data." << std::endl;
...
#undef DEFAULT_SEED
#undef DEFAULT_MAXIMUM_ITERATION
#undef DEFAULT_NUMBER_OF_RUN
#undef TIME_SEED
